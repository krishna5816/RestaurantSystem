...
namespace RestaurantSystem.Wastage.Purchaseitems
{
    public partial class List
    {
        ...
        private void Getbyitem(IQueryable<wastagepurchaseitem> wastagepurchaseitems)
        {
            betterListView1.AddColumns(new string[] { "Date", "Item", "Qty", "Unit" });
            foreach (var item in wastagepurchaseitems.ToList())
            {
                var b_list = new List<string>();
                b_list.Add(item.date.ToString());
                b_list.Add(item.purchaseitem.name.ToString());
